...
const Header = () => {
  ...
  const userCredit = useAppSelector((state) => state.user.users.userCredit);
  ...
  switch (modaltype) {
    ...
    case "Under_Maintenance":
      ModalContent = (
        <Maintenance fetchDate={true} closeModal={handelCloseModal} />
      );
      break;
    default:
      ModalContent = null;
    ...
    <>
      <div className="w-full mx-auto flex bg-gray-100 dark:bg-gray-800 px-5 py-3 justify-between items-center">
        ...
        <div className="lg:block hidden">
          ...
          <span className="dark:text-white text-black text-opacity-75 text-[.9rem] dark:text-opacity-60">
            Ding Ding CRM
          </span>
        </div>
        <div className="flex items-center space-x-4">
          <div className="relative pt-2">
            <button
              ...
              className="dark:text-white dark:hover:text-[#FFD117] hover:text-[#FFD117] text-gray-600 inline-block"
            >
              <Setting />
            </button>
            <div
              className={`${
                opensetting ? "scale-100 " : "scale-0"
              } transition-all rounded-xl bg-gray-200  p-2 z-[52] text-base min-w-[200px]  md:right-0 space-y-2 absolute top-[100%] dark:bg-gray-600`}
            >
              <button
                onClick={() => handelOpenModal("Add_Platform")}
                className="w-full py-1.5 dark:hover:bg-gray-500 hover:bg-gray-300 rounded-md dark:text-white"
              >
                Add Platform
              </button>
              <button
                onClick={() => handelOpenModal("Under_Maintenance")}
                className="w-full py-1.5  dark:hover:bg-gray-500 hover:bg-gray-300 rounded-md dark:text-white"
              >
                Under Maintenance
              </button>
          ...
          {user && (
            <div className="lg:flex hidden items-center space-x-1.5">
              <Profile />
              <span className="dark:text-white tracking-wide">
                {user?.username}
              </span>
              <span className="text-sm dark:text-gray-300 font-normal">
                ({user?.role})
              </span>
            </div>
          )}
      ...
      {opensetting && (
        <div
          onClick={() => setOpenSetting(!opensetting)}
          className="bg-black fixed top-0 bg-opacity-35 left-0 w-full h-screen z-[50]"
        ></div>
      )}
    </>
  );
